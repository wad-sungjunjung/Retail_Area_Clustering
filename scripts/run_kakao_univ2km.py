...
col = KakaoKeywordCollector(api_key=key, rate_limit_per_sec=10, radius_m=2000)
out_path = Path('data/interim/kakao_university_sc4_2km.jsonl')
existing = {}
if out_path.exists():
...
with open(out_path,'a',encoding='utf-8') as f:
    for _, r in feat[['sido','sigungu','eupmyeondong','lon','lat']].reset_index(drop=True).iterrows():
        ...
        rec={'sido':r.sido,'sigungu':r.sigungu,'eupmyeondong':r.eupmyeondong,'kakao_univ_sc4_2km':n or 0}
        f.write(json.dumps(rec,ensure_ascii=False)+'\n'); f.flush()
        cnt+=1
        if cnt%200==0: print(f'[2km] {cnt}/{len(feat)}')
rows=[]
with open(out_path) as f:
    for l in f: rows.append(json.loads(l))
pd.DataFrame(rows).to_parquet('data/interim/kakao_university_sc4_2km.parquet', index=False)
